...
export interface StreamCallback {
  ...
  onDone: (meta?: StreamEndMeta) => void;
...
function safeToString(value: any): string {
  ...
  try {
    return inspect(value, { depth: 4, breakLength: 120, maxArrayLength: 50 });
  } catch {
...
export class AzureOpenAIClient {
  ...
  private embeddingsHttpClient!: AxiosInstance;
  ...
  private embeddingsApiVersion!: string;
  private maxTokens!: number;
  private temperature!: number;
  private autocompleteMaxTokens!: number;
  ...
  reloadConfig() {
    ...
    const embeddingsDeploymentFromEnv =
      envGet('AZURE_OPENAI_EMBEDDINGS_DEPLOYMENT') || envGet('AZURE_OPENAI_EMBEDDINGS_DEPLOYMENT_NAME');
    const embeddingsApiVersionFromEnv = envGet('AZURE_OPENAI_EMBEDDINGS_API_VERSION');
    const embeddingsEndpointFromEnv =
      envGet('AZURE_OPENAI_EMBEDDINGS_ENDPOINT') || envGet('AZURE_OPENAI_EMBEDDINGS_BASE_URL');
    const embeddingsApiKeyFromEnv =
      envGet('AZURE_OPENAI_EMBEDDINGS_API_KEY') || envGet('AZURE_OPENAI_EMBEDDINGS_KEY');

    this.endpoint = (config.get<string>('endpoint', '') || endpointFromEnv).replace(/\/$/, '');
    this.apiKey = config.get<string>('apiKey', '') || apiKeyFromEnv;
    this.deploymentName = HARD_MODEL_SETTINGS.model;

    this.embeddingsEndpoint =
      (config.get<string>('embeddingsEndpoint', '') || embeddingsEndpointFromEnv || this.endpoint).replace(/\/$/, '');
    this.embeddingsApiKey = config.get<string>('embeddingsApiKey', '') || embeddingsApiKeyFromEnv || this.apiKey;
    this.embeddingsDeploymentName =
      config.get<string>('embeddingsDeploymentName', '') || embeddingsDeploymentFromEnv || this.deploymentName;
    this.apiVersion = config.get<string>('apiVersion', '') || apiVersionFromEnv || '2025-04-01-preview';
    this.embeddingsApiVersion =
      config.get<string>('embeddingsApiVersion', '') || embeddingsApiVersionFromEnv || '2023-05-15';
    ...
    this.embeddingsHttpClient = axios.create({
  ...
  getResolvedTargets(): {
    chat: { endpoint: string; deployment: string; apiVersion: string };
    embeddings: { endpoint: string; deployment: string; apiVersion: string };
  } {
    return {
      chat: {
  ...
  async pingChat(options?: ChatOptions): Promise<void> {
    if (!this.isConfigured()) throw new Error(this.getConfigError());
    // Reuse the same code path as the real chat UI to avoid schema drift.
    await this.chatToText([], 'ping', options);
  }

  async pingEmbeddings(options?: EmbeddingsOptions): Promise<number> {
  ...
  async chatToText(history: ChatMessage[], userMessage: ChatContent, options?: ChatOptions): Promise<string> {
    let out = '';
    await this.chat(
      history,
      userMessage,
  ...
  async embed(texts: string[], options?: EmbeddingsOptions): Promise<number[][]> {
    if (!this.isEmbeddingsConfigured()) {
      throw new Error(
        'Embeddings are not configured. Set azureCodex.embeddingsEndpoint/embeddingsApiKey/embeddingsDeploymentName ' +
    ...
    if (this.embeddingsDeploymentName === this.deploymentName && /\bgpt-?5\b|\bcodex\b/i.test(this.deploymentName)) {
      throw new Error(
        `Embeddings deployment is set to "${this.embeddingsDeploymentName}" which looks like a chat model. ` +
          `Set AZURE_OPENAI_EMBEDDINGS_DEPLOYMENT to an embeddings model (e.g. text-embedding-3-large).`
      );
    }
    const input = texts.map((t) => String(t ?? '').slice(0, 16_000));
    ...
    try {
      ...
      if (this._isCanceled(error)) throw error;
      ...
      try {
        ...
        if (this._isCanceled(legacyError)) throw legacyError;
  ...
  private _parseEmbeddingsResponse(json: any, expected: number): number[][] {
    const out = json?.data;
    if (!Array.isArray(out)) throw new Error('Unexpected embeddings response shape.');
    const embeddings = out.map((d: any) => d?.embedding);
    if (embeddings.length !== expected) throw new Error('Embeddings response length mismatch.');
    for (const e of embeddings) {
      if (!Array.isArray(e)) throw new Error('Unexpected embeddings response shape.');
    }
    return embeddings as number[][];
  }

  async completeInline(
    args: { language: string; prefix: string; suffix: string },
    callbacks: StreamCallback,
    options?: ChatOptions
  ): Promise<void> {
    ...
    const input = [
      {
        role: 'system',
        content:
          'You are an autocomplete engine. Return only the completion text to insert at the cursor. ' +
    ...
    await this._consumeSseStream(stream, callbacks, (json) => {
      ...
      if (typeof type === 'string' && type.endsWith('.delta') && typeof json?.delta === 'string') {
        if (type.includes('output_text') || type.includes('text')) return json.delta;
      }
      return null;
    }, options?.signal);
  }

  private _buildAbsoluteUrl(urlPath: string): URL {
  ...
  private async _postSseStream(urlPath: string, body: any, signal?: AbortSignal): Promise<IncomingMessage> {
      ...
      const req = lib.request(
        ...
        (res) => {
          ...
          if (status < 200 || status >= 300) {
            ...
            res.on('end', () => {
              const err: any = new Error(`HTTP ${status}: ${out.slice(0, 4000)}`);
              err.httpStatus = status;
              const retryAfter = res.headers?.['retry-after'];
              if (retryAfter) err.retryAfter = Array.isArray(retryAfter) ? retryAfter[0] : String(retryAfter);
              reject(err);
            });
            res.on('error', reject);
            return;
  ...
  private async _postJson(
    urlPath: string,
    body: any,
    opts: { signal?: AbortSignal; apiKey: string; baseEndpoint: string }
  ): Promise<any> {
  ...
  private _shouldPreferResponsesApi(): boolean {
  ...
  private _parseApiVersionDate(apiVersion: string): number | null {
  ...
  private _responsesUrl(): string {
    // Works with endpoints like:
    // - https://{resource}.openai.azure.com
    // - https://{resource}.cognitiveservices.azure.com
    // - https://{resource}.openai.azure.com/openai/v1
    if (/\/openai\/v1\/?$/.test(this.endpoint)) return `/responses`;
    return `/openai/v1/responses`;
  }

  private _embeddingsUrl(): string {
    if (/\/openai\/v1\/?$/.test(this.endpoint)) return `/embeddings`;
    return `/openai/v1/embeddings`;
  }

  private _legacyEmbeddingsUrl(): string {
    return `/openai/deployments/${this.embeddingsDeploymentName}/embeddings?api-version=${this.embeddingsApiVersion}`;
  }

  private _shouldFallbackToLegacyEmbeddings(error: any, formattedError: string): boolean {
  ...
  private async _chatResponsesWithRetries(
  ...
  private async _chatResponses(
    conversation: ChatMessage[],
    callbacks: StreamCallback,
    options: { includeTemperature: boolean },
    ...
    if (options.includeTemperature) body.temperature = this.temperature;
  ...
  private async _chatCompletionsWithRetries(
    conversation: ChatMessage[],
    callbacks: StreamCallback,
    options: ChatOptions | undefined,
    model: string
  ...
  private async _chatCompletions(
    conversation: ChatMessage[],
    callbacks: StreamCallback,
    options: { includeTemperature: boolean },
    requestOptions: ChatOptions | undefined,
  ...
  private async _consumeSseStream(
    ...
    callbacks: StreamCallback,
    extractToken: (json: any) => string | null,
    signal?: AbortSignal
  ): Promise<void> {
    return await new Promise<void>((resolve, reject) => {
      let buffer = '';
      const meta: StreamEndMeta = {};
      let done = false;
      ...
      const onData = (chunk: Buffer) => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (typeof fr === 'string' && fr) meta.finishReason = fr;

            const type = json?.type;
            if (typeof type === 'string' && type === 'response.completed') {
              const status = json?.response?.status;
              if (typeof status === 'string' && status === 'incomplete') meta.incomplete = true;
              const incompleteDetails = json?.response?.incomplete_details;
              if (incompleteDetails) meta.incomplete = true;
            }
          } catch (_) {
  ...
  private _isCanceled(error: any): boolean {
    const code = error?.code || error?.name;
    if (code === 'ERR_CANCELED' || code === 'CanceledError' || code === 'AbortError') return true;
    const msg = String(error?.message || '').toLowerCase();
    return msg.includes('canceled') || msg.includes('cancelled') || msg.includes('aborted');
  }

  private async _formatAxiosError(error: any): Promise<string> {
    const status = error?.response?.status;
    const requestId =
    ...
    if (!message) message = safeToString(error?.message || 'Unknown error');

    const prefixParts: string[] = [];
    if (typeof status === 'number') prefixParts.push(`HTTP ${String(status)}`);
    if (requestId) prefixParts.push(`requestId=${safeToString(requestId)}`);
  ...
  private _buildConnectivityHint(error: any, rawMessage: string): string {
    ...
    if (!(looksLikeConnRefused || looksLikeDns || looksLikeTimeout)) return '';

    const checks: string[] = [];
    if (host) checks.push(`verify host reachability to ${host}:443`);
  ...
  private async _extractErrorMessage(error: any): Promise<string> {
    ...
    if (data && typeof data === 'object' && typeof data.on === 'function') {
      const text = await this._readStreamToString(data as IncomingMessage);
  ...
  private async _readStreamToString(stream: IncomingMessage): Promise<string> {
  ...
  private _isUnsupportedParamError(message: string, paramName: string): boolean {
  ...
  private async _withRetry<T>(run: () => Promise<T>): Promise<T> {
    let attempt = 0;
    while (true) {
      attempt++;
      try {
  ...
  private _toResponsesContent(content: ChatContent): any {
    if (typeof content === 'string') return content;
    if (!Array.isArray(content)) return String(content);
    return content.map((p: any) => {
      const type = String(p?.type || '');
      if (type === 'text') return { type: 'input_text', text: p.text ?? '' };
      if (type === 'image_url') {
        const url = typeof p.image_url === 'string' ? p.image_url : p.image_url?.url;
        return { type: 'input_image', image_url: url ?? '' };
      }
      if (type === 'input_text') return { type: 'input_text', text: p.text ?? '' };
      if (type === 'input_image') return { type: 'input_image', image_url: p.image_url ?? '' };
      return p;
    });
  }

  private _toChatCompletionsContent(content: ChatContent): any {
    if (typeof content === 'string') return content;
    if (!Array.isArray(content)) return String(content);
    return content.map((p: any) => {
      const type = String(p?.type || '');
      if (type === 'input_text') return { type: 'text', text: p.text ?? '' };
      if (type === 'input_image') return { type: 'image_url', image_url: { url: p.image_url ?? '' } };
      if (type === 'text') return { type: 'text', text: p.text ?? '' };
      if (type === 'image_url') {
        const url = typeof p.image_url === 'string' ? p.image_url : p.image_url?.url;
  ...
  private _shouldFallbackToResponses(error: any, formattedError: string): boolean {
    const status = error?.response?.status;
    const msg = formattedError.toLowerCase();
    if (status === 404) return true;
    if (status !== 400) return false;
    return (
      msg.includes('chatcompletion') ||
  ...
  private _shouldFallbackToChatCompletions(error: any, formattedError: string): boolean {
    const status = error?.response?.status;
    const msg = formattedError.toLowerCase();
    if (status === 404) return true;
    if (status !== 400) return false;
  ...
  private _readDotEnvFromWorkspace(): Record<string, string> {
  ...
  private _parseDotEnv(raw: string): Record<string, string> {
