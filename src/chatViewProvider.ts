...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private _pinnedTextFiles: Map<string, string> = new Map();
  private _pinnedImages: Map<string, string> = new Map(); // path -> data URL
  ...
  private async _handleUserMessage(text: string, opts?: { displayText?: string }) {
    ...
    const autoContinueMaxTurns = Math.max(1, Math.min(10, config.get<number>('autoContinueMaxTurns', 4)));

    let fullResponse = '';
    let scratchHistory: ChatMessage[] = [...historyWithContext];
    let userContent: ChatContent = this._buildUserContent(text);
    ...
    if (hadError || abort.signal.aborted) return;
  ...
  private async _applyFileBlock(lang: string, code: string) {
  ...
  ): Promise<{ kind: 'file' | 'delete'; path: string } | null> {
  ...
  private _buildPinnedContext(maxChars: number): string {
    const items: string[] = [];
    if (this._pinnedTextFiles.size) {
      items.push(
        `Pinned text files (added via + / Fetch):\n` +
    ...
    if (this._pinnedImages.size) {
      items.push(
        `Pinned images (sent with your next message as image attachments):\n` +
  ...
  private _buildUserContent(text: string): any {
    if (!this._pinnedImages.size) return text;
    const parts: any[] = [{ type: 'input_text', text }];
  ...
  private async _pickAttachments() {
    ...
    for (const uri of picked) {
      ...
      this._pinnedTextFiles.set(rel, this._truncate(extracted, 200_000));
    }
  ...
  private async _attachmentToText(relPath: string, bytes: Uint8Array): Promise<string | null> {
    ...
    if (ext === 'xlsx' || ext === 'xls') {
      try {
        ...
        const parts: string[] = [`Attached file (excel -> csv preview): ${relPath}`];
  ...
  private async _removeContextItem(kind: string, label: string) {
    ...
    if (k === 'text') this._pinnedTextFiles.delete(l);
    if (k === 'image') this._pinnedImages.delete(l);
    this._sendContextStatus();
  }
  ...
  private async _fetchRequestedFiles(code: string) {
    ...
    for (const relPath of requested) {
      if (this._looksSensitive(relPath)) continue;
      const uri = vscode.Uri.joinPath(folders[0].uri, relPath);
      try {
        const bytes = await vscode.workspace.fs.readFile(uri);
        const text = Buffer.from(bytes).toString('utf8');
        this._pinnedTextFiles.set(relPath, this._truncate(text, 200_000));
        added.push(relPath);
      } catch {
        // ignore missing
      }
    ...
    await this._handleUserMessage(
      ...
      { displayText: `Fetched ${added.length} file(s) into context. Continue.` }
    );
  }
  ...
  private async _autoApplyIfEnabled(assistantText: string) {
    ...
    const applied: Array<{ kind: 'file' | 'delete'; path: string }> = [];
    const ops: Array<{ path: string; prevExists: boolean; prevContent: string }> = [];
    ...
    const picked = await vscode.window.showInformationMessage(
      `Azure Codex: Applied ${applied.length} change(s) (${applied.slice(0, 3).map((a) => a.path).join(', ')}${applied.length > 3 ? ', ...' : ''}).`,
      'Undo'
    );
    if (picked === 'Undo') await this._undoLastApply();
  }
        ...
        try {
          switch (name) {
            ...
            case 'run_command': {
              ...
              const res = await exec(command, { cwd: root, timeout: 120_000, maxBuffer: 2_000_000 });
