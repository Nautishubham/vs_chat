...
exports.SessionHistoryPanel = void 0;
...
class SessionHistoryPanel {
    static show(args) {
        ...
        const panel = vscode.window.createWebviewPanel('azureCodex.sessionHistory', 'Azure Codex: Session History', { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
            enableScripts: true,
        ...
        panel.onDidDispose(() => {
            if (SessionHistoryPanel._current === instance)
                SessionHistoryPanel._current = undefined;
        });
        return instance;
    }
    constructor(panel, entries, onRestore, onRefresh) {
        this._entries = [];
        this._panel = panel;
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (type === 'restore') {
                ...
                if (!id || !this._onRestore)
                    return;
                const ok = await vscode.window.showWarningMessage('Restore this snapshot? This reverts the files changed by that action.', { modal: true }, 'Restore');
                if (ok !== 'Restore')
                    return;
...
exports.SessionHistoryPanel = SessionHistoryPanel;
//# sourceMappingURL=sessionHistoryPanel.js.map