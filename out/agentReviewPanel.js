...
exports.AgentReviewPanel = void 0;
const vscode = __importStar(require("vscode"));
const diffUtil_1 = require("./diffUtil");
class AgentReviewPanel {
    static show(args) {
        ...
        const panel = vscode.window.createWebviewPanel('azureCodex.agentReview', args.title, { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
            enableScripts: true,
        ...
        panel.onDidDispose(() => {
            if (AgentReviewPanel._current === inst)
                AgentReviewPanel._current = undefined;
        });
        return inst;
    }
    constructor(panel, extensionUri) {
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (type === 'apply') {
                ...
                const confirm = await vscode.window.showWarningMessage(`Apply ${accepted.length} accepted change(s) to disk?`, { modal: true }, 'Apply');
                if (confirm !== 'Apply')
                    return;
            ...
            if (type === 'discard') {
                const confirm = await vscode.window.showWarningMessage('Discard all staged changes?', { modal: true }, 'Discard');
                if (confirm !== 'Discard')
                    return;
                this._discardCb?.();
                this._panel.dispose();
    ...
    _setSession(session) {
        ...
        this._files = staged.map((c) => {
            const d = (0, diffUtil_1.unifiedDiff)(c.prevContent, c.nextContent, c.path);
...
exports.AgentReviewPanel = AgentReviewPanel;
//# sourceMappingURL=agentReviewPanel.js.map